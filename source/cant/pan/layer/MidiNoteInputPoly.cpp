...
MidiNoteInputPoly::MidiNoteInputPoly(size_u numberVoices, id_u8 channel,
...
Optional<size_u> MidiNoteInputPoly::receive(const time_d tCurrent,
                                            const MidiNoteInputData &data) {
  /*
...
CANT_CONSTEXPR
static auto findClosestToneIndex = [](const Stream<MidiNoteInput> &notes,
...
Optional<size_u>
MidiNoteInputPoly::chooseVoice(const MidiNoteInputData &data) const {
  const bool inputIsPressed = data.isPressed();
  const tone_i8 inputTone = data.getToneNative();

  /*
   * The way it works in Pure Data when a note is received by the poly object
   * is:
   * - if it stopped being pressed (vel = 0) then we choose the voice with the
   * note's tone.
   * - if it started:
   * -- voice stealing -> we take next voice to the last voice we updated,
   ...
   * -- Mainly so that it disorients as little as possible the pitch-shifter it
   * is assigned to. Another difference is that we won't allow multiple notes
   * with the same tone.
   */
  /*
