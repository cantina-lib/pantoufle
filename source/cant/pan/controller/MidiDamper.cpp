...
MidiDamper::MidiDamper(const size_u numberVoices, const id_u8 channel,
                       const id_u8 controllerId)
...
bool MidiDamper::isOn(const MidiControlInternal &control) {
  return control.getValue() > c_midiControlMidValue;
}

void MidiDamper::beforeControlProcess(
  ...
  for (auto &shouldHold : m_shouldHoldNotes) {
    shouldHold = static_cast<id_u8>(static_cast<bool>(shouldHold) &&
                                    isOn(incomingControl));
  }
}

void MidiDamper::beforeNoteProcess(const MidiNoteInternal &incomingNote) {
  ...
  m_shouldHoldNotes.at(voice) = static_cast<id_u8>(
      isOn() && (static_cast<bool>(m_isMemoryPlaying.at(voice)) ||
                 incomingNote.isPlaying()));
}

void MidiDamper::IMPL_process(MidiNoteInternal &note) const {
   ...
   */
  note.setChangedPlaying(note.isPlaying() !=
                         static_cast<bool>(m_isMemoryPlaying.at(voice)));
}

UPtr<MidiController> MidiDamper::make(const size_u numberVoices,
                                      const id_u8 channel,
                                      const id_u8 controllerId) {
  return UPtr<MidiController>(
      new MidiDamper(numberVoices, channel, controllerId));
}
