...
MidiNoteInternal::MidiNoteInternal(const size_u voice)
    : MidiNote<MidiNoteInternalData>(voice), m_isPlaying(false),
      m_justChangedPlaying(false) {}

void MidiNoteInternal::set(const MidiNoteInput &input) {
  m_data = MidiNoteInternalData(input.getData());
  m_tStart = input.getStartingTime();
  m_isPlaying =
      input.isPlaying(); // at input stage, playing if and only if pressed
  m_justChangedPlaying = input.justChangedPlaying();
}

MidiNoteOutput::MidiNoteOutput(const size_u voice)
    : MidiNote<MidiNoteOutputData>(voice), m_isPlaying(false),
      m_justChangedPlaying(false), m_isSet(false) {}

void MidiNoteOutput::set(const MidiNoteInternal &internal) {
